 ...
 * @returns {string}
 */
export function getBoilerplateRemoval(document) {
  ...
  // TODO - Do we need this div still?
...
function getShadowRoot(node) {
  // TODO - Use the open or closed in Gecko.
  // return asElement(node)?.openOrClosedShadowRoot ?? null;
  return asElement(node)?.shadowRoot ?? null;
}
...
function asHTMLElement(node) {
  ...
  if (/** @type {any} */ (HTMLElement).isInstance) {
    if (/** @type {any} */ (HTMLElement).isInstance(node)) {
      return /** @type {HTMLElement} */ (node);
    }
  } else if (
 ...
 * content translation. This first node is the root of the page.
 *
...
/**
 * TODO - The original TranslationsDocument algorithm didn't require this.
 *
 * @param {Node} node
 * @param {Set<Node>} blocks
 */
