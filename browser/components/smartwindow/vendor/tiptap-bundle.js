    ...
    this.nodesBetween(
      ...
      (node, pos) => {
        let nodeText = node.isText
          ? node.text.slice(Math.max(from2, pos) - pos, to - pos)
          : !node.isLeaf
            ? ""
            : leafText
  ...
  */
  mark(marks) {
    return marks == this.marks
      ? this
      : new Node2(this.type, this.attrs, this.content, marks);
  }
  /**
...
function checkAttrs(attrs, values, type, name) {
  for (let name2 in values)
    if (!(name2 in attrs))
      throw new RangeError(
        `Unsupported attribute ${name2} for ${type} of type ${name2}`
      );
...
class DOMSerializer {
  ...
  */
  serializeMark(mark, inline2, options = {}) {
    ...
    return (
      toDOM &&
      renderSpec(doc$1(options), toDOM(mark, inline2), null, mark.attrs)
    );
  }
...
function renderSpec(doc2, structure, xmlNS, blockArraysIn) {
  ...
  let contentDOM;
  let dom = xmlNS
    ? doc2.createElementNS(xmlNS, tagName)
    : doc2.createElement(tagName);
...
class ReplaceAroundStep extends Step {
  ...
  */
  ...
  apply(doc2) {
    if (
      this.structure &&
      (contentBetween(doc2, this.from, this.gapFrom) ||
        contentBetween(doc2, this.gapTo, this.to))
    )
  ...
  map(mapping) {
    let from2 = mapping.mapResult(this.from, 1),
      to = mapping.mapResult(this.to, -1);
    let gapFrom =
      this.from == this.gapFrom ? from2.pos : mapping.map(this.gapFrom, -1);
...
function clearIncompatible(
    ...
    if (!allowed) {
      ...
      if (clearNewlines && child.isText && parentType.whitespace != "pre") {
        ...
        while ((m = newline.exec(child.text))) {
          ...
          replSteps.push(
            new ReplaceStep(cur + m.index, cur + m.index + m[0].length, slice2)
          );
        }
      }
    }
  ...
  for (let i = replSteps.length - 1; i >= 0; i--) tr2.step(replSteps[i]);
}
...
function setBlockType$1(tr2, from2, to, type, attrs) {
  ...
  tr2.doc.nodesBetween(from2, to, (node, pos) => {
    ...
    if (
      ...
      canChangeType(tr2.doc, tr2.mapping.slice(mapFrom).map(pos), type)
    ) {
      ...
      if (convertNewlines === false) replaceLinebreaks(tr2, node, pos, mapFrom);
      clearIncompatible(
        tr2,
        tr2.mapping.slice(mapFrom).map(pos, 1),
        type,
        void 0,
      ...
      if (convertNewlines === true) replaceNewlines(tr2, node, pos, mapFrom);
      return false;
    }
  });
}
...
function setNodeMarkup(tr2, pos, type, attrs, marks) {
  ...
  if (node.isLeaf) return tr2.replaceWith(pos, pos + node.nodeSize, newNode);
...
function join(tr2, pos, depth) {
  ...
  let $before = tr2.doc.resolve(pos - depth),
...
function replaceRange(tr2, from2, to, slice2) {
  if (!slice2.size) return tr2.deleteRange(from2, to);
  let $from = tr2.doc.resolve(from2),
  ...
  for (let i = targetDepths.length - 1; i >= 0; i--) {
    ...
    if (tr2.steps.length > startSteps) break;
...
function replaceRangeWith(tr2, from2, to, node) {
  if (
    !node.isInline &&
    from2 == to &&
    tr2.doc.resolve(from2).parent.content.size
  ) {
  ...
  */
  ...
  invert(doc2) {
    return new AttrStep(
      this.pos,
      this.attr,
      doc2.nodeAt(this.pos).attrs[this.attr]
    );
  }
  map(mapping) {
    let pos = mapping.mapResult(this.pos, 1);
...
class DocAttrStep extends Step {
  ...
  */
  ...
  apply(doc2) {
    let attrs = /* @__PURE__ */ Object.create(null);
    for (let name in doc2.attrs) attrs[name] = doc2.attrs[name];
    attrs[this.attr] = this.value;
...
class Selection {
  ...
  replace(tr2, content = Slice.empty) {
    ...
    for (let i = 0; i < ranges.length; i++) {
      let { $from, $to } = ranges[i],
        mapping = tr2.mapping.slice(mapFrom);
      tr2.replaceRange(
  ...
  */
  static atEnd(doc2) {
    return (
      findSelectionIn(doc2, doc2, doc2.content.size, doc2.childCount, -1) ||
      new AllSelection(doc2)
    );
  }
  /**
...
class TextSelection extends Selection {
  ...
  */
  ...
  replace(tr2, content = Slice.empty) {
    ...
    if (content == Slice.empty) {
      ...
      if (marks) tr2.ensureMarks(marks);
    }
  }
  eq(other) {
    return (
  ...
  */
  static fromJSON(doc2, json) {
    ...
    return new TextSelection(
      doc2.resolve(json.anchor),
      doc2.resolve(json.head)
    );
  }
  /**
...
class TextBookmark {
  ...
  resolve(doc2) {
    return TextSelection.between(
      doc2.resolve(this.anchor),
      doc2.resolve(this.head)
    );
  }
}
...
class NodeBookmark {
  ...
  resolve(doc2) {
    let $pos = doc2.resolve(this.anchor),
      node = $pos.nodeAfter;
  ...
  */
  ...
  replace(tr2, content = Slice.empty) {
    if (content == Slice.empty) {
      ...
      if (!sel.eq(tr2.selection)) tr2.setSelection(sel);
    } else {
      super.replace(tr2, content);
    }
  }
...
function selectionToInsertionEnd$1(tr2, startLen, bias) {
  ...
  if (!(step instanceof ReplaceStep || step instanceof ReplaceAroundStep))
    return;
  let map2 = tr2.mapping.maps[last],
    end;
...
class Transaction extends Transform {
  ...
  */
  ensureMarks(marks) {
    if (
      !Mark$1.sameSet(this.storedMarks || this.selection.$from.marks(), marks)
    )
      ...
      node = node.mark(
        this.storedMarks ||
          (selection.empty
            ...
            : selection.$from.marksAcross(selection.$to) || Mark$1.none)
      );
  ...
  insertText(text, from2, to) {
    ...
    if (from2 == null) {
      ...
      if (!marks) {
        ...
        marks =
          to == from2 ? $from.marks() : $from.marksAcross(this.doc.resolve(to));
      }
    ...
    for (let i = 0; i < this.config.plugins.length; i++)
      if (i != ignore) {
        ...
        if (
          ...
          !plugin.spec.filterTransaction.call(plugin, tr2, this)
        )
          return false;
      }
    return true;
...
function textNodeBefore$1(node, offset2) {
  for (;;) {
    if (node.nodeType == 3 && offset2) return node;
...
function textNodeAfter$1(node, offset2) {
  for (;;) {
    if (node.nodeType == 3 && offset2 < node.nodeValue.length) return node;
...
function scrollRectIntoView(view, rect, startDOM) {
  ...
  for (let parent = startDOM || view.dom; ; ) {
    ...
    if (rect.top < bounding.top + getSide$1(scrollThreshold, "top"))
      ...
      rect.bottom >
      bounding.bottom - getSide$1(scrollThreshold, "bottom")
    )
      moveY =
...
function posFromCaret(view, node, offset2, coords) {
  ...
  return outsideBlock > -1
    ? outsideBlock
    : view.docView.posFromDOM(node, offset2, -1);
}
...
function posAtCoords(view, coords) {
  ...
  if (caret) ({ node, offset: offset2 } = caret);
  ...
  if (node) {
    ...
    if (
      ...
      offset2 &&
      node.nodeType == 1 &&
      (prev = node.childNodes[offset2 - 1]).nodeType == 1 &&
...
function coordsAtPos(view, pos, side) {
  ...
  if (node.nodeType == 3) {
    if (
      ...
      return flattenV(
        singleRect(textRange(node, from2, to), takeSide),
        takeSide < 0
      );
    }
  }
    ...
    for (let offset2 = 0, i = 0; i < this.children.length; i++) {
      ...
      if (
        ...
        if (from2 >= startInside && to <= endInside) {
          this.dirty =
            from2 == offset2 || to == end ? CONTENT_DIRTY : CHILD_DIRTY;
          if (
            from2 == startInside &&
            to == endInside &&
...
class MarkViewDesc extends ViewDesc {
  ...
  static create(parent, mark, inline2, view) {
    ...
    if (!spec || !spec.dom)
      spec = DOMSerializer.renderSpec(
        document,
        mark.type.spec.toDOM(mark, inline2),
        null,
        mark.attrs
      );
  ...
  slice(from2, to, view) {
    ...
    let nodes = this.children,
      size2 = this.size;
    if (to < size2) nodes = replaceNodes(nodes, to, size2, view);
...
class NodeViewDesc extends ViewDesc {
  ...
  static create(parent, node, outerDeco, innerDeco, view, pos) {
    ...
    if (node.isText) {
      ...
      let spec2 = DOMSerializer.renderSpec(
        document,
  ...
  updateChildren(view, pos) {
    let inline2 = this.node.inlineContent,
      off = pos;
...
function selectionFromDOM(view, origin = null) {
  ...
  if (selectionCollapsed(domSel)) {
    ...
    if (
      ...
      [anchor, head] =
        max2 == view.state.selection.anchor ? [max2, min2] : [min2, max2];
...
function skipIgnoredNodesBefore(view) {
  ...
  if (
    ...
    isIgnorable(node.childNodes[offset2], -1)
  )
    force = true;
  for (;;) {
    if (offset2 > 0) {
...
function stopNativeHorizontalDelete(view, dir) {
  ...
  if (nextNode && !nextNode.isText) {
    ...
    if (dir < 0) tr2.delete($head.pos - nextNode.nodeSize, $head.pos);
    else tr2.delete($head.pos, $head.pos + nextNode.nodeSize);
    view.dispatch(tr2);
    return true;
  }
  return false;
...
function serializeForClipboard(view, slice2) {
  ...
  let text =
    view.someProp("clipboardTextSerializer", f => f(slice2, view)) ||
...
function addToSibling(wrap2, lastWrap, node, sibling, depth) {
  if (
    ...
    if (
      match.matchType(depth == wrap2.length - 1 ? node.type : wrap2[depth + 1])
    )
      return sibling.copy(
...
function defaultTripleClick(view, inside, event) {
  ...
  for (let i = $pos.depth + 1; i > 0; i--) {
    ...
    if (node.inlineContent)
      updateSelection(
        view,
        TextSelection.create(doc2, nodePos + 1, nodePos + 1 + node.content.size)
      );
...
class MouseDown {
  constructor(view, pos, event, flushed) {
    ...
    this.target =
      targetDesc && targetDesc.dom.nodeType == 1 ? targetDesc.dom : null;
...
function doPaste(view, text, html, preferPlain, event) {
  ...
  view.dispatch(
    tr2.scrollIntoView().setMeta("paste", true).setMeta("uiEvent", "paste")
  );
  return true;
}
...
editHandlers.drop = (view, _event) => {
  ...
  if (move) {
    let { node } = dragging;
    if (node) node.replace(tr2);
    else tr2.deleteSelection();
  }
  ...
  if (isNode2) tr2.replaceRangeWith(pos, pos, slice2.content.firstChild);
  else tr2.replaceRange(pos, pos, slice2);
  ...
  if (
    ...
    tr2.mapping.maps[tr2.mapping.maps.length - 1].forEach(
  ...
  forChild(offset2, node) {
    ...
    for (let i = 0; i < this.local.length; i++) {
      ...
      if (dec.from < end && dec.to > start && dec.type instanceof InlineType) {
        let from2 = Math.max(start, dec.from) - start,
...
class DecorationGroup {
  ...
  map(mapping, doc2) {
    const mappedDecos = this.members.map(member =>
      member.map(mapping, doc2, noSpec)
    );
...
function buildTree(spans, node, offset2, options) {
  ...
  let locals = moveSpans(hasNulls ? withoutNulls(spans) : spans, -offset2).sort(
    byPos
  );
...
function parseBetween(view, from_, to_) {
  ...
  if (find2 && find2[0].pos != null) {
    let anchor2 = find2[0].pos,
      head = find2[1] && find2[1].pos;
      ...
      if (
        ...
        !(
          (chrome &&
            ...
            (sel2.head == chFrom || sel2.head == tr2.mapping.map(chTo) - 1)) ||
          (ie$1 && sel2.empty && sel2.head == chFrom)
        )
      )
  ...
  if (inlineChange) {
    if ($from.pos == $to.pos) {
      ...
      let marks = doc2
        ...
        .marksAcross(doc2.resolve(change.endA));
      if (marks) tr2.ensureMarks(marks);
      view.dispatch(tr2);
    } else if (
      ...
      if (markChange.type == "add") tr2.addMark(chFrom, chTo, markChange.mark);
      else tr2.removeMark(chFrom, chTo, markChange.mark);
      view.dispatch(tr2);
    } else if (
...
function resolveSelection(view, doc2, parsedSel) {
  ...
  return selectionBetween(
    view,
    doc2.resolve(parsedSel.anchor),
    doc2.resolve(parsedSel.head)
  );
}
  ...
  */
  ...
  updateDraggedNode(dragging, prev) {
    ...
    this.dragging = new Dragging(
      dragging.slice,
      dragging.move,
      found2 < 0 ? void 0 : NodeSelection.create(this.state.doc, found2)
    );
  }
  someProp(propName, f) {
...
EditorView.prototype.dispatch = function (tr2) {
  ...
  if (dispatchTransaction) dispatchTransaction.call(this, tr2);
  else this.updateState(this.state.apply(tr2));
};
...
for (var code in base)
  if (!shift$2.hasOwnProperty(code)) shift$2[code] = base[code];
...
function normalize(map2) {
  ...
  for (let prop in map2) copy2[normalizeKeyName$1(prop)] = map2[prop];
  return copy2;
}
        ...
        if (dispatch) {
          ...
          tr2.setSelection(
            textblockAt(before, "end")
              ? Selection.findFrom(
                  tr2.doc.resolve(tr2.mapping.map($cut.pos, -1)),
                  -1
                )
              : NodeSelection.create(tr2.doc, $cut.pos - before.nodeSize)
          );
      ...
      if (dispatch) {
        ...
        tr2.setSelection(
          textblockAt(after, "start")
            ? Selection.findFrom(tr2.doc.resolve(tr2.mapping.map($cut.pos)), 1)
            : NodeSelection.create(tr2.doc, tr2.mapping.map($cut.pos))
        );
        dispatch(tr2.scrollIntoView());
      }
      return true;
...
const exitCode$1 = (state, dispatch) => {
  ...
  if (dispatch) {
    let pos = $head.after(),
      tr2 = state.tr.replaceWith(pos, pos, type.createAndFill());
...
function splitBlockAs(splitNode) {
  return (state, dispatch) => {
    ...
    if (!atEnd && atStart && $from.node(splitDepth).type != deflt) {
      let first2 = tr2.mapping.map($from.before(splitDepth)),
        $first = tr2.doc.resolve(first2);
      if (
        deflt &&
        $from
          .node(splitDepth - 1)
    ...
    if (dispatch) dispatch(tr2.scrollIntoView());
    return true;
  };
}
...
function wrapInList$1(listType, attrs = null) {
  return function (state, dispatch) {
    ...
    if (!wrapRangeInList(tr2, range, listType, attrs)) return false;
    if (dispatch) dispatch(tr2.scrollIntoView());
    return true;
  };
}
function wrapRangeInList(tr2, range, listType, attrs = null) {
  ...
  if (
    ...
    if (range.endIndex < range.parent.childCount)
      range = new NodeRange(
        range.$from,
        doc2.resolve(range.$to.end(range.depth)),
        range.depth
      );
    doJoin = true;
  }
  ...
  if (tr2) doWrapInList(tr2, range, wrap2, doJoin, listType);
  return true;
}
...
function liftToOuterList(state, dispatch, itemType, range) {
  ...
  if (end < endOfList) {
    ...
    range = new NodeRange(
      tr2.doc.resolve(range.$from.pos),
      tr2.doc.resolve(endOfList),
      range.depth
    );
  }
...
function liftOutOfList(state, dispatch, range) {
  ...
  let $start = tr2.doc.resolve(range.start),
  ...
  if (node.content.childCount) {
    ...
    node.forEach((child, offset2) => {
      ...
      (_a = output.content) == null
        ? void 0
        : _a.push(getDebugJSON(child, startOffset + offset2 + increment));
    });
  }
  if (node.text) {
...
function getMarkRange($pos, type, attributes) {
  ...
  attributes =
    attributes || ((_a = start.node.marks[0]) == null ? void 0 : _a.attrs);
...
var isAtEndOfNode = (state, nodeType) => {
  ...
  if (nodeType) {
    const parentNode2 = findParentNode(node => node.type.name === nodeType)(
      state.selection
    );
    if (!parentNode2) {
      return false;
...
var cut =
  ...
  ({ editor, tr: tr2 }) => {
    ...
    tr2.setSelection(
      new TextSelection(tr2.doc.resolve(Math.max(newPos - 1, 0)))
    );
    return true;
  };
...
var focus =
  ...
  ({ editor, view, tr: tr2, dispatch }) => {
    ...
    const selection =
      resolveFocusPosition(tr2.doc, position) || editor.state.selection;
...
var setContent =
  ...
  ({ editor, tr: tr2, dispatch, commands }) => {
    const { doc: doc2 } = tr2;
    if (parseOptions.preserveWhitespace !== "full") {
      ...
      if (dispatch) {
        tr2
          .replaceWith(0, doc2.content.size, document2)
  ...
  ({ state, dispatch, chain }) => {
    ...
    return chain()
      ...
      .command(({ state: updatedState }) => {
        return setBlockType(type, { ...attributesToCopy, ...attributes })(
          updatedState,
          dispatch
        );
      })
...
var toggleList =
  ...
  ({ editor, tr: tr2, state, dispatch, chain, commands, can }) => {
    ...
    if (range.depth >= 1 && parentList && range.depth - parentList.depth <= 1) {
      ...
      if (
        ...
        return chain()
          ...
          .command(() => joinListBackwards(tr2, listType))
          .command(() => joinListForwards(tr2, listType))
          .run();
      }
    }
          ...
          nextTransaction.before.nodesBetween(
            ...
            (node, from2) => {
              ...
              const isFullyWithinRange =
                change.oldRange.from <= from2 && to <= change.oldRange.to;
          ...
          commands.command(({ tr: tr2 }) => {
            ...
            const isAtStart =
              parentIsIsolating && $parentPos.parent.childCount === 1
                ...
                : Selection.atStart(doc2).from === pos;
            if (
              !empty2 ||
...
var NodePos = class _NodePos {
  ...
  get children() {
    ...
    this.node.content.forEach((node, offset2) => {
      ...
      children.push(
        new _NodePos($pos, this.editor, isBlock, isBlock ? node : null)
      );
    });
    return children;
  }
...
var Editor = class extends EventEmitter {
  constructor(options = {}) {
    ...
    this.on("drop", ({ event, slice: slice2, moved }) =>
      this.options.onDrop(event, slice2, moved)
    );
    this.on("paste", ({ event, slice: slice2 }) =>
  ...
  dispatchTransaction(transaction) {
    ...
    const mostRecentFocusTr = transactions.findLast(
      tr2 => tr2.getMeta("focus") || tr2.getMeta("blur")
    );
    const focus2 =
    ...
    if (
      ...
      !transactions.some(tr2 => tr2.docChanged) ||
  ...
  return new InputRule({
    ...
    handler: ({ state, range, match }) => {
      ...
      if (match[1]) {
        ...
        tr2
          .insert(insertionStart, config.type.create(attributes))
          .delete(tr2.mapping.map(start), tr2.mapping.map(end));
      }
      tr2.scrollIntoView();
    },
    undoable: config.undoable,
        ...
        props: {
          handlePaste: (view, event) => {
            ...
            if (tr2.selection.$from.parent.type !== this.type) {
              tr2.setSelection(
                TextSelection.near(
                  tr2.doc.resolve(Math.max(0, tr2.selection.from - 2))
                )
              );
            }
              ...
              if (dispatch) {
                ...
                if ($to.nodeAfter) {
                  if ($to.nodeAfter.isTextblock) {
                    tr2.setSelection(
                      TextSelection.create(tr2.doc, $to.pos + 1)
                    );
                  ...
                  const node =
                    (_a = $to.parent.type.contentMatch.defaultType) == null
                      ? void 0
                      : _a.create();
                  if (node) {
                    tr2.insert(posAfter, node);
                    tr2.setSelection(
                      TextSelection.create(tr2.doc, posAfter + 1)
                    );
                  }
                }
                tr2.scrollIntoView();
      ...
      if (groups) {
        if (nextState.t && typeof nextState.t === "string") {
          const allFlags = Object.assign(
            flagsForToken(nextState.t, groups),
            flags
          );
...
function registerCustomProtocol(scheme2, optionalSlashSlash = false) {
  if (INIT.initialized) {
    warn(
      `linkifyjs: already initialized - will not register custom scheme "${scheme2}" ${warnAdvice}`
    );
  }
  ...
  return new Plugin({
    ...
    props: {
      handleClick: (view, pos, event) => {
        ...
        const href =
          (_a = link == null ? void 0 : link.href) != null ? _a : attrs.href;
        const target =
          (_b = link == null ? void 0 : link.target) != null
            ? _b
            : attrs.target;
...
var Append = /* @__PURE__ */ (function (RopeSequence3) {
  ...
  Append2.prototype.forEachInvertedInner = function forEachInvertedInner(
    ...
    if (
      to < leftLen &&
      this.left.forEachInvertedInner(f, Math.min(from2, leftLen), to, start) ===
        false
    ) {
      return false;
    }
      ...
      (item, i) => {
        ...
        if (remap) {
          ...
          if (step && transform.maybeStep(step).doc) {
            ...
            addAfter.push(
              new Item(map2, void 0, void 0, addAfter.length + addBefore.length)
            );
          }
          mapFrom--;
...
function applyTransaction(history2, state, tr2, options) {
  let historyTr = tr2.getMeta(historyKey),
    rebased;
  ...
  if (tr2.steps.length == 0) {
    ...
    tr2.getMeta("addToHistory") !== false &&
    ...
    let newGroup =
      ...
      (!appended &&
        ...
        (history2.prevTime < (tr2.time || 0) - options.newGroupDelay ||
          !isAdjacentTo(tr2, history2.prevRanges)));
    let prevRanges = appended
      ? mapRanges(history2.prevRanges, tr2.mapping)
      : rangesFor(tr2.mapping.maps);
    return new HistoryState(
      history2.done.addTransform(
        tr2,
    ...
    return new HistoryState(
      history2.done.rebased(tr2, rebased),
    ...
    return new HistoryState(
      history2.done.addMaps(tr2.mapping.maps),
      history2.undone.addMaps(tr2.mapping.maps),
      mapRanges(history2.prevRanges, tr2.mapping),
      history2.prevTime,
      history2.prevComposition
    );
...
function buildCommand(redo2, scroll) {
  return (state, dispatch) => {
    ...
    if (dispatch) {
      ...
      if (tr2) dispatch(scroll ? tr2.scrollIntoView() : tr2);
    }
    return true;
  };
}
...
const size$1 = function (options) {
  ...
  return {
    ...
    async fn(state) {
      ...
      const { apply: apply2 = () => {}, ...detectOverflowOptions } = evaluate(
        options,
        state
      );
      const overflow = await detectOverflow$1(state, detectOverflowOptions);
...
function isContainingBlock(elementOrCss) {
  ...
  return (
    ...
    (!webkit2 && (css.filter ? css.filter !== "none" : false)) ||
...
function getInnerBoundingClientRect(element, strategy) {
  const clientRect2 = getBoundingClientRect(
    element,
    true,
    strategy === "fixed"
  );
...
function hasFixedPositionAncestor(element, stopNode) {
  ...
  if (
    parentNode2 === stopNode ||
    !isElement(parentNode2) ||
    isLastTraversableNode(parentNode2)
  ) {
    return false;
  }
  return (
    getComputedStyle$1(parentNode2).position === "fixed" ||
    hasFixedPositionAncestor(parentNode2, stopNode)
  );
}
...
var CellSelection = class _CellSelection extends Selection {
  ...
  content() {
    ...
    for (let row = rect.top; row < rect.bottom; row++) {
      const rowContent = [];
      for (
        let index = row * map2.width + rect.left, col = rect.left;
        col < rect.right;
        col++, index++
      ) {
    ...
    if (anchorRect.top <= headRect.top) {
      ...
      if (headRect.bottom < map2.height)
        $headCell = doc2.resolve(
          tableStart +
            map2.map[map2.width * (map2.height - 1) + headRect.right - 1]
        );
    } else {
      if (headRect.top > 0)
      ...
      if (anchorRect.bottom < map2.height)
        $anchorCell = doc2.resolve(
          tableStart +
            map2.map[map2.width * (map2.height - 1) + anchorRect.right - 1]
        );
    }
  ...
  eq(other) {
    return (
      other instanceof _CellSelection &&
  ...
  static rowSelection($anchorCell, $headCell = $anchorCell) {
    ...
    if (anchorRect.left <= headRect.left) {
      ...
      if (headRect.left > 0)
        $headCell = doc2.resolve(
          tableStart + map2.map[headRect.top * map2.width]
        );
...
var CellBookmark = class _CellBookmark {
  ...
  resolve(doc2) {
    const $anchorCell = doc2.resolve(this.anchor),
      $headCell = doc2.resolve(this.head);
    if (
...
var BubbleMenuView = class {
  constructor({
    ...
    this.shouldShow = ({ view: view2, state, from: from2, to }) => {
      ...
      const { empty: empty2 } = selection;
      const isEmptyTextBlock =
        !doc2.textBetween(from2, to).length && isTextSelection(state.selection);
    ...
    this.blurHandler = ({ event }) => {
      ...
      if (
        (event == null ? void 0 : event.relatedTarget) === this.editor.view.dom
      ) {
        return;
      }
      this.hide();
  ...
  show() {
    ...
    (_b = (_a = this.appendTo) != null ? _a : this.view.dom.parentElement) ==
    null
      ? void 0
      : _b.appendChild(this.element);
  ...
  findChildren(doc2, node => node.type.name === name).forEach(block => {
    ...
    const nodes =
      ...
      (languages.includes(language) ||
        registered(language) ||
        ((_a = lowlight.registered) == null
          ? void 0
          : _a.call(lowlight, language)))
        ? getHighlightNodes(
