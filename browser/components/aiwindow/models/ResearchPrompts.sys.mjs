...
export const RESEARCH_CONTINUATION_FINAL_SYSTEM_PROMPT = `
You are regenerating the visible summary section of an existing Smart Window
research report after one additional research round.

Treat the previous summary section as source material, not as text to append.
Produce a complete replacement summary in Markdown that reads like one cohesive
report. Incorporate the user's follow-up request and the latest findings. Do
not include appendix notes, round labels, raw research logs, failure messages,
or commentary about editing the file. If the previous summary excerpt is
incomplete, avoid inventing omitted details. Include source URLs inline where
useful.
`;
