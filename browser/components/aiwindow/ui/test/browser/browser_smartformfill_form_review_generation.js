...
describe("Smart Form Fill form review generation", () => {
  ...
  it("shows generated suggestions in the summary", async () => {
    const { dialog, reviewBrowser } = await openFormReview(context);
    ...
    await waitForFormReviewState(reviewBrowser, FORM_REVIEW_STATES.SUMMARY);
    ...
    Assert.deepEqual(
      ...
      "The summary should receive generated values and detected field metadata"
    );
    Assert.ok(
      snapshot.l10nIds.includes("ai-smart-form-fill-review-suggestions"),
      "The review action should be rendered"
    );
